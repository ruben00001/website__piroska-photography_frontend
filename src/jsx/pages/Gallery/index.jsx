...
class Gallery extends Component {
  ...
  componentDidMount() {
    ...
    axios
      ...
      .then(_ => {
        this.props.defineImageDetails(
          this.state.displayFilterValue,
          this.state.numImages
        );
      });
    axios.get(`${strapiAPI}/tags`).then(response => {
      this.setState({
  ...
  onImageLoad = e => {
    this.setState(
      {
        imagesTotalHeight:
          this.state.imagesTotalHeight + e.currentTarget.offsetHeight,
        imageHeights: [
          ...this.state.imageHeights,
      ...
      _ => {
        if (this.state.numImagesLoaded === this.state.numImages) {
          ...
          setTimeout(() => {
            this.setState({
              imgContainerHeight:
                this.state.imagesTotalHeight /
                  this.state.imageContainerVars.columns +
                this.state.imageContainerVars.extraspace,
              initialLoad: false
            });
          }, 200);
          ...
          setTimeout(() => {
            // allow for refresh of page when images finished loading
            this.setState({
  ...
  filterImages = filter => {
    ...
    if (filter === 'category') {
      ...
      filteredImages =
        filter === 'newest'
          ? this.state.images.concat().sort((a, b) => {
    ...
    imagesContainerHeight =
      filteredImagesHeight / this.state.imageContainerVars.columns +
      this.state.imageContainerVars.extraspace;
      ...
      this.setState(
        {
          ...
          filteredImages:
            filter === 'none' ? this.state.images : filteredImages,
  ...
  refreshPage = () => {
    // for now refresh page when width of image container changes (i.e when window changes size)
        ...
        >
          {props => (
            <div style={props} className='gallery-page_container'>
              ...
              >
                <div
                  className='gallery-page_filter_line gallery-page_filter_line--1'
                  onClick={() =>
                    this.state.displayFilterValue
                ...
                >
                  {this.state.showFilterOptions ? (
                    <FontAwesomeIcon
                      className='gallery-page_filter_icon gallery-page_filter_icon--box'
                      icon={faMinusSquare}
                    ></FontAwesomeIcon>
                  ) : (
                    <FontAwesomeIcon
                  ...
                  <h3 className='gallery-page_filter_title'>
                    {this.state.currentFilterValue ? 'un' : null}filter
                  </h3>
                </div>
                        ...
                        <div className='gallery-page_filter_second-line_options_option'>
                          ...
                          {this.state.showDateFilter && (
                            <div className='gallery-page_filter_second-line_options_option_content gallery-page_filter_second-line_options_option_content--1'>
                              <div
                                className='gallery-page_filter_item gallery-page_filter_item--1'
                                onClick={_ => {
                                  this.filterImages('newest');
                                }}
                              >
                                <FontAwesomeIcon
                                  className='gallery-page_filter_icon gallery-page_filter_icon--eye'
                                  icon={faEye}
                                ></FontAwesomeIcon>
                                <div>most recent</div>
                              </div>
                            ...
                            <div className='gallery-page_filter_second-line_options_option_content gallery-page_filter_second-line_options_option_content--2'>
                              {this.state.tags &&
                                this.state.tags.map(tag => (
                                  <div
                                    className='gallery-page_filter_item gallery-page_filter_item--2'
                                    onClick={_ => {
                                      this.setState(
                                        { currentFilterValue: tag.name },
                                        _ => this.filterImages('category')
                                      );
                                    }}
                                    key={tag.id}
                                    value={tag.name}
...
export default withLoadingScreen(
  withZoom(withRouter(Gallery), 'Gallery'),
  '/gallery'
);
